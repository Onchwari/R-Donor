...
<div class="px-20">
    <form action="{{url('donor/update-data1')}}"method="post"enctype="multipart/form-data"class="px-20 pt-4 -ml-24 text-center ">
    @csrf
        @method('PUT')
        <h2 class="text-3xl font-semibold text-red-700 font-serif">PROFILE</h2><br>
        <div class="py-2">
        <img class="rounded-full h-20 w-20  items-center ml-96 "src="/assests/users/{{$user->Photo}}">
        ...
        <div class="py-2">
            ...
            <input class="border-2 border-zinc-500 bg-slate-200 mr-14 rounded hover:border-orange-600 focus:border-2 focus:border-orange-600" type="text"name="Bloodgroup"value="{{(old('Bloodgroup'))?old('Bloodgroup'):$user->Bloodgroup}}">
        ...
        <button class="bg-red-700 rounded w-48 px-8 py-1.5 shadow-md hover:bg-orange-600 text-lg -mt-12 mr-12 object-center text-center float-right ">Edit</button>
    </form>
    <!-- <div>
    <button class="bg-red-700 rounded w-48 px-8 py-1.5 shadow-md hover:bg-orange-600 text-lg -mt-12 mr-12 object-center text-center float-right show-modal">Edit</button>
